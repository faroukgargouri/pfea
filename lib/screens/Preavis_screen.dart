...
class _ClientGroup {
  ...
  final String? raison; // ici on affiche nomClient si dispo
...
class PreavisScreen extends StatefulWidget {
  final String rep;
  const PreavisScreen({super.key, required this.rep});

  @override
  State<PreavisScreen> createState() => _PreavisScreenState();
}

class _PreavisScreenState extends State<PreavisScreen> {
  // ---- intl gating ----
  ...
  // Consider "date set" if not empty and not the .NET default 01/01/0001 (or ISO 0001-01-01…)
  ...
  /// Pour stats impayés: on considère impayé si status == '11' ou contient 'impay'
  ...
  int get _nbClientsDistincts =>
      _filtered.map((e) => e.codeClient).toSet().length;
    ...
    for (final entry in map.entries) {
      final items = entry.value
        ..sort((a, b) {
          ...
          // second clé: numReg / numCheque lexicographique pour stabilité (null-safe)
          return (cmp != 0)
              ? cmp
              : (b.numReg ?? '').compareTo(a.numReg ?? '');
        });
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<void>(
      ...
      builder: (context, snap) {
        ...
        return Scaffold(
          // ✅ Single AppBar: white bar + blue strip (text-only) with actions on the right
          appBar: TrikiAppBar(
            fullName: _fullName,
            codeSage: _codeSage,
            actionsBeforeLogout: [
            ...
            blueNavItems: const [
              BlueNavItem(label: 'LISTE PRÉAVIS PAR CLIENT', selected: true),
            ],
                  ...
                  builder: (context, snap) {
                    ...
                    if (_filtered.isEmpty) {
                      return const _EmptyState(
                        ...
                        subtitle: 'Aucun client/préavis ne correspond à votre recherche.',
  ...
  Widget _totalCard({required IconData icon, required String title, required String value}) {
    return Container(
